...
class UGrid(object):
    ...
    def from_ncfile(klass, nc_url, mesh_name=None):
        ...
        return load_grid_from_nc(nc_url, mesh_name)
...
def load_grid_from_nc(filename, mesh_name=None):
    """
    loads UGrid object from a netcdf file
    ...
    ## create a UGRid object to put all this in:
    grid = UGrid(mesh_name=mesh_name)

    ## Grab node coordinates from mesh meta-variable, and pull out the coord values
    try:
        node_coordinates = mesh_var.node_coordinates.strip().split()
    except AttributeError:
        raise ValueError("Unstructured meshes must include node coordinates, specified with the 'node_coordinates' attribute")

    num_node = len(nc.variables[node_coordinates[0]])
    nodes = np.empty((num_node, 2), dtype=np.float64)
    for coord in node_coordinates:
        try:
            units = nc.variables[coord].units
        except AttributeError:
            raise ValueError("Node coordinate doesn't contain 'units' attribute")
        if 'north' in units:
            nodes[:,1] = ncvars[coord][:]
        elif ('east' in units) or ('west' in units):
            nodes[:,0] = ncvars[coord][:]
        else:
            raise ValueError("Node coordinates 'units' attribute doesn't contain 'north', 'east', or 'west'") 
    
    grid.nodes = nodes

    ## Load the face connectivity array
    try:
        faces = nc.variables[mesh_var.face_node_connectivity][:,:]
        # if [3,nodes] instead of [nodes,3], transpose the array
        # logic below will fail for 3 element grids
        if faces.shape[0] == 3:
            faces = faces.T
        ## note: this ignores the start_index attribute!
        index_base = np.min(faces)
        if index_base  >= 1:
            faces -= index_base
        grid.faces = faces
    except KeyError:
    ...
    ## Load the edge connectivity array
    try:
        edges = nc.variables[mesh_var.edge_node_connectivity][:,:]
        # if [2,nodes] instead of [nodes,2], transpose the array
        # logic below will fail for 2 edge grids
        if edges.shape[0] == 2:
            edges = edges.T
        ## note: this ignores the start_index attribute!
        index_base = np.min(edges)
        if index_base  >= 1:
            edges -= index_base
        grid.edges = edges
    except KeyError:
    ...
    ## Load the face-face connectivity array
    try:
        var = nc.variables[mesh_var.face_face_connectivity]
        face_face = var[:,:]
        # if [3,faces] instead of [faces,3], transpose the array
        # logic below will fail for 3 edge grids
        if face_face.shape[0] == 3:
            face_face = face_face.T
        ## note: this ignores the start_index attribute!
        try:
            ...
            start_index = 0
        if start_index  >= 1:
            face_face -= start_index
            # check for flag value
            try:
                ## fixme: this won't work for more than one flag value
                flag_value = var.flag_values
                face_face[face_face==flag_value-start_index] = flag_value
            except AttributeError:
                pass
        grid.face_face_connectivity = face_face
    except KeyError:
        pass ## OK not to have this...

    ## Load the boundaries connectivity array
    try:
        var = nc.variables[mesh_var.boundary_node_connectivity]
        bounds = var[:,:]
        ## transpose Fortran-ordered arrays
        if bounds.shape[1]!=2:
            bounds = bounds.T
        try:
            ## fixme: should be be doing this for all grid indexes?
            start_index = var.start_index
        except AttributeError:
            start_index = 0
        if start_index  >= 1:
    ...
    return grid # Return dictionary of ugrid objects



def open_cf_todict( filename ):
    """
    read a netcdf or opendap url, and load all the grids into a dict of ugrid objects.
    """
    import netCDF4
    nc = netCDF4.Dataset(filename, 'r')
    grids = {}
    for name in find_mesh_names(nc):
        grids[name] = load_grid_from_nc(nc, name)
